...
def create_prompt(task_data, instance_input):
    ...
    prompt_parts.extend([f"Now complete the following example—", f"input: {instance_input}", f"output:"])
...
def process_translation(
    ...
    data_path = base_dir / Path(data_path)
    out_path = base_dir / Path(out_path)
    category_path = out_path / "translation"
    os.makedirs(category_path, exist_ok=True)

    prompt_length_dict = {}

    bare_prompts = os.path.join(category_path, "translation_data.jsonl")
    few_shot_prompts = os.path.join(category_path, "translation_data_few_shot.jsonl")

    if os.path.exists(bare_prompts) and os.path.exists(few_shot_prompts) and not overwrite:
        logger.info("Translation data already exists")
            ...
            if file_name.endswith(".json"):
                ...
                with open(file_path, "r") as json_file:
                    try:
                        ...
                        if "translation" in current_categories:
