...
@dataclass
class MetricCfg:
    behavior: str = field(default="toxicity", metadata={"help": "Which behaviour to evaluate"})
    metric: Optional[str] = field(
        default=None,
        metadata={"help": "Which metric to use. If none is given, the metric is loaded using the behavior string."},
    )
...
@dataclass
class EvalArgs:
    epochs: int = field(default=30, metadata={"help": "How many times to go though the whole dataset"})
    num_samples: int = field(default=500, metadata={"help": "How many samples to select from dataset"})
    num_bins: int = field(default=10, metadata={"help": "How many bins to divide the metric into"})
...
class ModelCfg:
    ...
    gen_batch_size: int = field(default=8, metadata={"help": "Batch size for generation."})
    ...
    class model_kwargs:
        ...
        load_in_4bit: bool = field(default=True, metadata={"help": "Whether to load model in 4 bit."})
...
class TrainCfg:
    @dataclass
    class EarlyStopping:
        patience: int = field(default=10, metadata={"help": "Patience in training algorithm."})
        delta: float = field(default=0.0, metadata={"help": "Delta in training algorithm."})
    ...
    epochs: int = field(
        default=100,
        metadata={"help": "Epochs to train regression network for."},  # They did 500 each
    )
    ...
    net_batch_size: int = field(default=100, metadata={"help": "Batch size of regression network."})
